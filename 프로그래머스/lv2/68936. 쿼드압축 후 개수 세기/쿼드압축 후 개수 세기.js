function check(arr){ // 압축 가능한 배열인지 확인하는 함수
    ...
    if(arr.toString()===array0.toString()){// 0으로 압축 가능한 경우
        return 0; 
    }
    if(arr.toString()===array1.toString()){// 1로 압축 가능한 경우
        return 1;
    }
    return -1; // 압축 불가능한 경우 -1 리턴
}

function divide(arr,cnt0,cnt1){ // 배열을 4개로 쪼개는 함수
    ...
    while(i<2*half){
        ...
        while(k<2*half){     
            ...
            if(newArr[0].length===1){ //더 이상 쪼갤 수 없는 경우
                if(newArr[0][0] === 0 ){ 
                    cnt0++;
                }else{
                    cnt1++;
